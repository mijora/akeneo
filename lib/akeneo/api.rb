...
module Akeneo
  class API
    ...
    def initialize(url:, client_id:, secret:, username:, password:, access_token: nil, refresh_token: nil, last_refresh: nil)
      @url = url
      authorization_service.authorize!(client_id: client_id, secret: secret, username: username, password: password, access_token: access_token, refresh_token: refresh_token, last_refresh: last_refresh)
    end
    ...
    def current_refresh_token
      authorization_service.refresh_token
    end
    ...
    def products(with_family: nil, with_completeness: nil, updated_after: nil, options: {}, identifier: [])
      product_service.all(
    ...
    def products_paged(with_family: nil, with_completeness: nil, updated_after: nil, options: {}, identifier: [], next_path: nil)
    ...
    def update_products(options:)
      product_service.update_products(options)
    end
    ...
    def attribute_options(code)
      attribute_service.options(code)
    end
